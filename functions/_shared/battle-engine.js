/* ============================================================================
   ...
   Phase 6 responsibility: normalize owned cards with effective stats and apply
   type matchup power modifiers during battle simulation.
   ============================================================================ */
...
import { getCardTypeSummary, getTypeMatchup, normalizeCardType } from './type-config.js';
...
export const mockBattleEncounters = [
  { id: 'training-yard-goblin', name: 'Training Yard Goblin', difficulty: 'Easy', element: 'Starter', enemyType: 'neutral', enemyPower: 86, staminaCost: 4, rewardGold: 120, rewardXp: 35, description: 'A safe first fight tuned for a full starter squad under the current stat budget system.' },
  { id: 'calendar-hydra', name: 'Calendar Hydra', difficulty: 'Medium', element: 'Pressure', enemyType: 'shadow', enemyPower: 132, staminaCost: 7, rewardGold: 260, rewardXp: 80, description: 'A mid-tier pressure fight tuned for upgraded commons, uncommons, or mixed-rarity squads.' },
  { id: 'storm-forge-wyrm', name: 'Storm Forge Wyrm', difficulty: 'Hard', element: 'Boss', enemyType: 'flame', enemyPower: 205, staminaCost: 10, rewardGold: 520, rewardXp: 150, description: 'A boss-style prototype tuned for rare-heavy squads under the current stat budget system.' },
];
...
function flattenCardPayload(row) {
  ...
  const level = toNumber(payload.level ?? payload.card_level ?? payload.cardLevel ?? payload.progression?.level, 1);
  ...
  return {
    ...
    normalized: {
      ...
      category: String(payload.category || typeSummary.label || 'Vault'),
      type: cardType,
      cardType,
      card_type: cardType,
...
function applyEncounterMatchup(card, encounter) {
  ...
  const matchup = getTypeMatchup(card.type, enemyType);
...
function selectDefaultSquad(cards) { return [...cards].filter((card) => card.eligible).sort((a, b) => b.baseBattlePower - a.baseBattlePower).slice(0, maxBattleSquadSize); }
function selectRequestedSquad(cards, requestedIds) { const lookup = buildCardLookup(cards); return requestedIds.map((cardId) => ({ requestedId: cardId, card: lookup.get(cardId) || null })); }
...
function sanitizeForId(value) { return String(value || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'unknown'; }
...
function buildCombatLog({ encounter, squad, squadPower, enemyPower, victory, margin }) {
  ...
  const matchupLine = squad.map((card) => `${card.name}: ${card.typeLabel || card.type} vs ${card.enemyType} ${card.matchupResult} (${card.baseBattlePower}→${card.battlePower})`).join('; ');
  return [`${leader} leads ${squad.length} card(s) into ${encounter.name}.`, `Squad power resolves to ${squadPower} after type matchups.`, `${encounter.name} answers with ${encounter.enemyType || 'neutral'} pressure at enemy power ${enemyPower}.`, matchupLine ? `Type matchups: ${matchupLine}.` : 'Type matchups were neutral.', victory ? `The squad wins the battle contract by ${margin} power.` : `The squad falls short in the battle contract by ${Math.abs(margin)} power.`, 'Battle history may be written only by POST /api/battles. Phase 6 uses effective stats and type matchups.'];
}

function buildSimulation({ ownerUserId, encounter, squad, createdAt }) {
  const matchupSquad = squad.map((card) => applyEncounterMatchup(card, encounter));
  const squadPower = matchupSquad.reduce((total, card) => total + card.battlePower, 0);
  const enemyPower = encounter.enemyPower;
  const margin = squadPower - enemyPower;
  const victory = margin >= 0;
  const rewardPreview = buildRewardPreview(encounter, victory, matchupSquad);
  return { simulationId: buildSimulationId(ownerUserId, encounter.id, matchupSquad), ownerUserId, ownerDisplayName: ownerUserId === temporaryBattleUserId ? temporaryBattleUserDisplayName : ownerUserId, encounter: { ...encounter, enemyType: normalizeCardType(encounter.enemyType || encounter.element || 'neutral') }, squad: matchupSquad, squadPower, enemyPower, margin, victory, rewardPreview, xpPreview: buildXpPreview(matchupSquad, rewardPreview.xpPerCard ?? rewardPreview.totalXp), combatLog: buildCombatLog({ encounter, squad: matchupSquad, squadPower, enemyPower, victory, margin }), resultRule: 'effective stats plus type matchup modifier compared against mock encounter enemyPower', createdAt, writes: [] };
}
...
export async function resolveBattleSimulation(env, { ownerUserId = temporaryBattleUserId, encounterId = mockBattleEncounters[0].id, squadCardIds = [], createdAt = new Date().toISOString() } = {}) {
  ...
  if (!validation.ok) return { ok: false, status: 400, phase: 'battle-6', readOnly: true, error: 'Battle simulation validation failed.', ownerUserId, encounterId, requestedSquadCardIds: requestedIds, ownedCardsScanned: ownedCards.length, battleEligibleCount: eligibleCards.length, validation, writes: [], notes: ['This endpoint performs no writes even when validation fails.', 'Use /api/battle-inventory to inspect eligible card ids before supplying squadCardIds.'] };
  return { ok: true, status: 200, phase: 'battle-6', readOnly: true, source: 'D1 owned Vault cards plus frontend mock encounter contract', requested: { ownerUserId, encounterId, squadCardIds: requestedIds }, ownedCardsScanned: ownedCards.length, battleEligibleCount: eligibleCards.length, defaultSquadUsed: requestedIds.length === 0, validation, simulation: buildSimulation({ ownerUserId, encounter, squad, createdAt }), availableEncounters: mockBattleEncounters.map((mockEncounter) => ({ id: mockEncounter.id, name: mockEncounter.name, enemyPower: mockEncounter.enemyPower, enemyType: mockEncounter.enemyType, difficulty: mockEncounter.difficulty })), guardrails: ['No battle_history write in simulation mode.', 'No reward write.', 'No XP or level write.', 'No currency write.', 'No stamina or energy write.', 'No Vault write.', 'No auth change.'], nextStep: 'POST /api/battles writes rewards only after Phase 6 simulation validation passes.' };
}
...
export async function ensureBattleHistorySchema(env) { await env.DB.prepare(battleHistorySql).run(); await env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_battle_history_user_created ON battle_history (user_id, created_at)').run(); await env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_battle_history_encounter_created ON battle_history (encounter_id, created_at)').run(); }
...
export async function writeBattleHistory(env, simulationResult, { now = new Date().toISOString() } = {}) {
  ...
  const resultJson = JSON.stringify({ battleId, phase: 'battle-6', ownerUserId: simulation.ownerUserId, ownerDisplayName: simulation.ownerDisplayName, encounterId: simulation.encounter.id, encounterName: simulation.encounter.name, enemyType: simulation.encounter.enemyType, victory: simulation.victory, squadPower: simulation.squadPower, enemyPower: simulation.enemyPower, margin: simulation.margin, squad: simulation.squad.map((card) => ({ cardId: card.id, sourceRowId: card.sourceRowId, cardTitle: card.name, rarity: card.rarity, type: card.type, matchup: card.matchup, level: card.level, maxLevel: card.maxLevel, baseBattlePower: card.baseBattlePower, battlePower: card.battlePower, stats: card.stats })), rewardPreview: simulation.rewardPreview, xpPreview: simulation.xpPreview, combatLog: simulation.combatLog, resultRule: simulation.resultRule, writes: ['battle_history'], deferredWrites: ['rewards', 'XP', 'level-ups', 'currency', 'stamina', 'energy', 'Vault changes'], createdAt: now });
  await ensureBattleHistorySchema(env);
  await env.DB.prepare(`INSERT INTO battle_history (id, user_id, encounter_id, victory, squad_power, enemy_power, result_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(battleId, simulation.ownerUserId, simulation.encounter.id, simulation.victory ? 1 : 0, simulation.squadPower, simulation.enemyPower, resultJson, now).run();
  return { battleId, historyRow: { id: battleId, userId: simulation.ownerUserId, encounterId: simulation.encounter.id, victory: simulation.victory, squadPower: simulation.squadPower, enemyPower: simulation.enemyPower, createdAt: now }, resultJson, simulation };
}
...
export async function readBattleHistory(env, { ownerUserId = temporaryBattleUserId, limit = 20 } = {}) {
  ...
  const battles = (result.results || []).map((row) => { let parsedResult = null; try { parsedResult = JSON.parse(row.resultJson || 'null'); } catch { parsedResult = null; } return { id: row.id, userId: row.userId, ownerUserId: row.userId, ownerDisplayName: row.userId === temporaryBattleUserId ? temporaryBattleUserDisplayName : row.userId, encounterId: row.encounterId, encounterName: parsedResult?.encounterName || row.encounterId, enemyType: parsedResult?.enemyType || 'neutral', victory: Boolean(row.victory), squadPower: Number(row.squadPower), enemyPower: Number(row.enemyPower), margin: parsedResult?.margin ?? Number(row.squadPower) - Number(row.enemyPower), squad: parsedResult?.squad || [], rewardPreview: parsedResult?.rewardPreview || null, xpPreview: parsedResult?.xpPreview || [], combatLog: parsedResult?.combatLog || [], writes: parsedResult?.writes || ['battle_history'], deferredWrites: parsedResult?.deferredWrites || ['rewards', 'XP', 'level-ups', 'currency', 'stamina', 'energy', 'Vault changes'], createdAt: row.createdAt, resultHydrated: Boolean(parsedResult) }; });
