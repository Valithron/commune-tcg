/* ============================================================================
   ...
   Battle Phase 8 responsibility: validate battle, require a unique attempt ID,
   apply gold and owned-card XP/level progression once, then write battle_history.
   Drops, pull tickets, stamina, energy, Vault grants, and auth remain deferred.
   ============================================================================ */
...
function validateAttemptId(attemptId) {
  if (!attemptId) {
    return 'battle-attempt-required';
  }

  if (!/^[a-zA-Z0-9_-]{8,120}$/.test(attemptId)) {
    return 'battle-attempt-invalid-format';
  }
  ...
  try {
    ...
    if (attemptError) {
      return jsonResponse({
        ...
        writes: [],
        error: 'Battle attempt validation failed.',
        code: attemptError,
        guardrails: [
          'Phase 8 requires a valid attemptId before any reward write.',
    ...
    return jsonResponse({
      ...
      requested: {
        ...simulationResult.requested,
        attemptId,
      },
      guardrails: [
        'Validated battle result was required before writes.',
        'A unique attemptId was required before writes.',
        'Gold was applied to user_resources only.',
        'XP and levels were applied to owned card rows only.',
        'Battle history was written with attemptId, reward, and XP application details.',
      ...
      nextStep: 'Verify duplicate attempt blocking before adding saved squads, stamina, drops, tickets, or battle UI polish.',
    });
  } catch (error) {
    if (error.code === 'duplicate-battle-attempt') {
      return jsonResponse({
        ...
        existingBattleId: error.existingBattleId,
        guardrails: [
          'Duplicate battle attempt detected before rewards were applied again.',
          'No gold, XP, level, stamina, energy, Vault, or card ownership writes occurred on the duplicate request.',
        ],
      }, { status: 409 });
    }
