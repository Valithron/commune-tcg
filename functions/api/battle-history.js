...
async function readBattleHistoryRows(env, { ownerUserId, ownerDisplayName, limit }) {
  ...
  const battles = (result.results || []).map((row) => { const parsedResult = parseResultJson(row.resultJson); return { id: row.id, attemptId: parsedResult?.attemptId || null, userId: row.userId, ownerUserId: row.userId, ownerDisplayName: row.userId === ownerUserId ? ownerDisplayName : row.userId, encounterId: row.encounterId, encounterName: parsedResult?.encounterName || row.encounterId, victory: Number(row.victory) === 1, squadPower: Number(row.squadPower), enemyPower: Number(row.enemyPower), margin: parsedResult?.margin ?? Number(row.squadPower) - Number(row.enemyPower), squad: parsedResult?.squad || [], rewardPreview: parsedResult?.rewardPreview || null, rewardApplied: parsedResult?.rewardApplied || null, xpPreview: parsedResult?.xpPreview || [], xpApplied: parsedResult?.xpApplied || parsedResult?.xpPreview || [], duplicateProtection: parsedResult?.duplicateProtection || null, combatLog: parsedResult?.combatLog || [], writes: parsedResult?.writes || ['battle_history'], deferredWrites: parsedResult?.deferredWrites || [], phase: parsedResult?.phase || 'battle-3', createdAt: row.createdAt, resultHydrated: Boolean(parsedResult) }; });
...
export async function onRequestGet({ env, request }) {
  ...
  try {
    ...
    return jsonResponse({ ok: true, phase: 'auth-current-user-battle-history', readOnly: true, source: 'D1 battle_history', ownerUserId, ownerDisplayName, ...history, notes: ['This endpoint performs no writes.', 'Battle history rows are written only by POST /api/battles.', 'Battle history rows may include attemptId, duplicateProtection, rewardApplied, and xpApplied details.', 'Pull tickets, drops, stamina, energy, Vault grants, and auth changes remain deferred.'] });
  } catch (error) {
