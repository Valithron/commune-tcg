/* ============================================================================
   API Admin Cards Endpoint
   Adds a contained admin editor for Library card rows in the cards table.
   ============================================================================ */
...
function toStat(value, fallback = 1) {
  ...
  return Math.min(Math.max(Math.round(parsed), 1), 99);
}
...
function normalizeCrop(value) {
  ...
  return {
    x: toCropNumber(crop.x ?? crop.left ?? 50, 50, 0, 100),
    y: toCropNumber(crop.y ?? crop.top ?? 50, 50, 0, 100),
    zoom: toCropNumber(crop.zoom ?? crop.z ?? crop.scale ?? 1, 1, 1, 3),
  };
}
...
function normalizeCardRow(row) {
  ...
  return {
    ...
    stats: {
      pow: toStat(payload.pow ?? stats.pow, 1),
...
async function getCardRow(env, id) {
  return env.DB.prepare('SELECT * FROM cards WHERE id = ? LIMIT 1').bind(String(id || '')).first();
}
...
async function maybeStoreImage(env, cardId, imageFile) {
  ...
  if (!env.CARD_IMAGES) {
    throw new Error('R2 binding CARD_IMAGES is not available for image replacement.');
  }

  const extension = getImageExtension(imageFile);

  if (!extension) {
    throw new Error('Replacement image must be PNG, JPG, or WEBP.');
  }

  if (imageFile.size > 8 * 1024 * 1024) {
    throw new Error('Replacement image must be 8 MB or smaller.');
  }
...
function fieldsFromPayload(payload, existingCard, storedImageKey = '') {
  ...
  return {
    ...
    rarity,
    stats: {
      pow: toStat(payload.pow ?? existingCard.stats.pow, 1),
    ...
    imageKey,
    crop: {
      x: toCropNumber(payload.crop_x ?? payload.x ?? existingCard.crop.x, existingCard.crop.x, 0, 100),
      y: toCropNumber(payload.crop_y ?? payload.y ?? existingCard.crop.y, existingCard.crop.y, 0, 100),
      zoom: toCropNumber(payload.crop_zoom ?? payload.zoom ?? existingCard.crop.zoom, existingCard.crop.zoom, 1, 3),
    },
  };
}
...
export async function onRequestGet({ env, request }) {
  ...
  return jsonResponse({
    ...
    warnings: ['Admin card editor can edit or delete rows in the cards table.'],
  });
}
...
export async function onRequestPost({ env, request }) {
  ...
  try {
    ...
    if (!row) {
      return errorResponse('Card was not found.', 404);
    }
    ...
    await env.DB.prepare(`
      ...
      SET character_id = ?,
          ...
          updated_at = ?
      WHERE id = ?
    `).bind(fields.characterId, nextJson, now, id).run();
    ...
    return jsonResponse({
      ...
      source: 'D1 cards',
      action: 'update',
      card: normalizeCardRow(updatedRow),
      warnings: storedImageKey ? ['Replacement image was stored in R2; old image cleanup is not automatic.'] : [],
    });
  } catch (error) {
...
export async function onRequestDelete({ env, request }) {
  ...
  try {
    ...
    return jsonResponse({
      ...
      deletedCardId: id,
      deletedImageKey: '',
      warnings: card.imageKey ? ['Card row deleted. R2 image was left in place to avoid deleting art that may be shared by submissions or other rows.'] : [],
    });
  } catch (error) {
