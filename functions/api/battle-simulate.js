/* ============================================================================
   ...
   Performs no battle_history, rewards, XP, currency, stamina, Vault, or auth writes.
   ============================================================================ */
...
function getEncounterById(encounterId) {
  return mockEncounters.find((encounter) => encounter.id === encounterId) || null;
}

function parseSquadCardIds(value) {
  return String(value || '')
    .split(',')
    .map((id) => id.trim())
    .filter(Boolean);
...
function buildValidation({ encounter, requestedIds, selectedEntries, squad }) {
  ...
  if (requestedIds.length > maxSquadSize) {
  ...
  return {
    ...
    missingIds,
    ineligibleCards,
    expectedSquadSize: maxSquadSize,
...
function buildCombatLog({ encounter, squad, squadPower, enemyPower, victory, margin }) {
  ...
  return [
    ...
    victory
      ? `The squad wins the no-write simulation by ${margin} power.`
      : `The squad falls short in the no-write simulation by ${Math.abs(margin)} power.`,
    'No battle history, rewards, XP, currency, stamina, or Vault data were written.',
  ];
}

function buildSimulation({ ownerUserId, encounter, squad }) {
...
export async function onRequestGet({ env, request }) {
  ...
  const encounterId = url.searchParams.get('encounterId') || mockEncounters[0].id;
  const requestedIds = parseSquadCardIds(url.searchParams.get('squadCardIds'));
  ...
  try {
    ...
    return jsonResponse({
      ...
      validation,
      simulation: buildSimulation({ ownerUserId, encounter, squad }),
      availableEncounters: mockEncounters.map((mockEncounter) => ({
        id: mockEncounter.id,
      ...
      nextStep: 'Battle Phase 3 should only add battle_history writes after this simulation contract is verified.',
    });
  } catch (error) {
