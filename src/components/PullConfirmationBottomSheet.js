/* ============================================================================
   Pull Confirmation Bottom Sheet
   Responsibility: render and initialize the pull confirmation sheet, including
   option selection, odds toggle, drag-to-close, and confirmed pull routing.
   ============================================================================ */
...
function getPullStatus({ balance, selectedCount }) {
  const selectedOption = getPullOption(selectedCount);
  const shortfall = Math.max(0, selectedOption.ticketCost - balance);

  return shortfall === 0
    ? 'Ready'
    : `Need ${shortfall} more ticket${shortfall === 1 ? '' : 's'}`;
}
...
export function renderPullConfirmationBottomSheet({ selectedCount, resources, sheetOpen }) {
  const safeSelectedCount = clampPullCount(selectedCount || 5);
  const balance = Number(resources?.tickets || 0);
  const selectedOption = getPullOption(safeSelectedCount);
  const canAfford = balance >= selectedOption.ticketCost;

  return `
    <div
      class="pull-sheet-overlay${sheetOpen ? ' is-open' : ''}"
      data-pull-sheet
      data-selected-count="${safeSelectedCount}"
      data-ticket-balance="${balance}"
    ...
    >
      <div class="pull-sheet" data-pull-sheet-panel role="dialog" aria-modal="true" aria-labelledby="pull-confirm-title">
        ...
        <div class="pull-sheet-status" data-pull-status>${getPullStatus({ balance, selectedCount: safeSelectedCount })}</div>
...
function setPullHash(selectedCount, closed) {
...
function updateOddsPanel(overlay) {
...
function setConfirmWorking({ confirmButton, confirmLabel, status, isWorking, errorMessage = '' }) {
  if (!confirmButton) {
    return;
  }

  confirmButton.disabled = isWorking;
  confirmButton.classList.toggle('button-working', isWorking);

  if (confirmLabel) {
    confirmLabel.textContent = isWorking ? 'Opening...' : 'Confirm Pull';
  }

  if (status) {
    status.textContent = isWorking ? 'Preparing reveal...' : errorMessage;
  }
}
...
export function initPullConfirmationBottomSheet(root) {
  ...
  function updateSelection(nextCount) {
    ...
    const canAfford = balance >= getPullOption(selectedCount).ticketCost;
    if (status) {
      status.textContent = getPullStatus({ balance, selectedCount });
    }
    if (confirmButton) {
  ...
  function closeSheet(updateHash = true) {
    ...
    if (updateHash) {
      setPullHash(selectedCount, true);
    }
  ...
  updateOddsPanel(overlay);

  confirmButton?.addEventListener('click', async () => {
    if (balance < getPullOption(selectedCount).ticketCost) {
      return;
    }

    setConfirmWorking({ confirmButton, confirmLabel, status, isWorking: true });

    try {
      await executeConfirmedPull(selectedCount);
    } catch (error) {
      setConfirmWorking({
        confirmButton,
        confirmLabel,
