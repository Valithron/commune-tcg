      ...
      <div class="admin-card-modal-layout">
        ...
        <aside class="admin-card-preview-column">
          <div class="admin-card-preview-shell">
            ...
            <div class="admin-card-editor-section admin-card-preview-crop-panel">
              <span class="section-kicker">Crop</span>
              <div class="admin-card-editor-crop" data-admin-crop-controls>
                <label><span>Crop X</span><input form="${escapeHtml(formId)}" name="crop_x" type="range" min="0" max="100" step="1" value="${escapeHtml(crop.x)}" /></label>
                <label><span>Crop Y</span><input form="${escapeHtml(formId)}" name="crop_y" type="range" min="0" max="100" step="1" value="${escapeHtml(crop.y)}" /></label>
                <label><span>Zoom</span><input form="${escapeHtml(formId)}" name="crop_zoom" type="range" min="1" max="3" step="0.01" value="${escapeHtml(crop.zoom)}" /></label>
              </div>
            </div>

            <div class="admin-card-editor-section admin-card-preview-upload-panel">
              <span class="section-kicker">Replace Art</span>
              <label><span>Image Upload</span><input form="${escapeHtml(formId)}" name="image" type="file" accept="image/png,image/jpeg,image/webp" /></label>
              <p class="empty-note">Image key is generated by storage and is not editable here.</p>
            </div>
...
function updatePreview(modal) {
  ...
  if (image) {
    const x = modal.querySelector('[name="crop_x"]')?.value || 50;
    const y = modal.querySelector('[name="crop_y"]')?.value || 50;
    const zoom = modal.querySelector('[name="crop_zoom"]')?.value || 1;
