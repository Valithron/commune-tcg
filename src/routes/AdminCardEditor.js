...
function sortValue(card, key) { if (key === 'pow') return cardStat(card, 'pow'); if (key === 'def') return cardStat(card, 'def'); if (key === 'spd') return cardStat(card, 'spd'); if (key === 'creatorDisplayName') return creatorName(card).toLowerCase(); if (key === 'createdAt' || key === 'updatedAt') return new Date(card?.[key] || 0).getTime() || 0; if (key === 'type') return typeLabel(card?.[key]).toLowerCase(); return String(card?.[key] ?? '').toLowerCase(); }
...
function renderTableHeader() { return `<tr><th>Art</th><th>${renderSortButton('Name', 'name')}</th><th>${renderSortButton('Character', 'characterId')}</th><th>${renderSortButton('Type', 'type')}</th><th>${renderSortButton('Rarity', 'rarity')}</th><th>${renderSortButton('POW', 'pow')}</th><th>${renderSortButton('DEF', 'def')}</th><th>${renderSortButton('SPD', 'spd')}</th><th>${renderSortButton('Creator', 'creatorDisplayName')}</th><th>${renderSortButton('Updated', 'updatedAt')}</th><th>Actions</th></tr>`; }
function renderTable() { return `<div class="glass-panel admin-card-table-panel"><div class="admin-card-table-toolbar"><div><span class="section-kicker">Library Rows</span><h2 class="section-title">${adminCardsCache.length} editable cards</h2></div><span class="empty-note">Click any row to edit. Click a column header to sort.</span></div><div class="admin-card-table-scroll"><table class="admin-card-table" data-admin-card-table><thead>${renderTableHeader()}</thead><tbody data-admin-card-table-body>${renderTableRows() || '<tr><td colspan="11" class="empty-note">No cards were found in the cards table.</td></tr>'}</tbody></table></div></div>`; }
...
export async function renderAdminCardEditor() {
  ...
  return `<section class="hero-panel admin-card-manager-hero"><span class="section-kicker">Admin Cards</span><h2 class="hero-title">Manage the Library.</h2><p class="hero-copy">A desktop-first card manager for scanning, sorting, editing, replacing art, adjusting crop, and deleting D1 Library card rows. Card type editing now uses the seven Commune types.</p><div class="action-row"><a class="button button-secondary" href="#/admin">Admin Home</a><a class="button button-secondary" href="#/admin/submissions">Submissions</a><span class="status-pill">${escapeHtml(payload.source || 'D1 cards')}</span></div></section><section class="admin-card-manager" data-admin-card-editor>${renderTable()}<div class="admin-card-modal" data-admin-card-modal aria-hidden="true"></div></section>`;
}
...
function renderRowsInto(root) { const tableBody = root.querySelector('[data-admin-card-table-body]'); const tableHead = root.querySelector('[data-admin-card-table] thead'); if (tableBody) tableBody.innerHTML = renderTableRows() || '<tr><td colspan="11" class="empty-note">No cards were found in the cards table.</td></tr>'; if (tableHead) tableHead.innerHTML = renderTableHeader(); }
