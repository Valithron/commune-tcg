/* ============================================================================
   ...
   Phase 6 responsibility: shared helpers for selecting, saving, loading, and
   resolving backend-owned battle squads with optional type matchup adjustment.
   ============================================================================ */

import { fetchJson, getApiRoutes } from './apiClient.js';
import { applyTypeMatchups } from './typeMatchups.js';
...
export function createBattleAttemptId() {
  const randomPart = globalThis.crypto?.randomUUID ? globalThis.crypto.randomUUID().replace(/-/g, '').slice(0, 16) : Math.random().toString(36).slice(2, 14);
...
export function buildBattleResultsHref({ encounterId, squadCardIds = [], attemptId } = {}) {
  ...
  if (encodedSquad) params.set('squadCardIds', encodedSquad);
...
export async function saveBattleSquad({ ownerUserId = defaultBattleOwnerUserId, squadCardIds = [] } = {}) {
  const routes = getApiRoutes();
  const response = await fetch(routes.battleSquad, { method: 'POST', headers: { accept: 'application/json', 'content-type': 'application/json' }, body: JSON.stringify({ ownerUserId, squadCardIds: encodeSquadCardIds(squadCardIds) }) });
  const payload = await response.json().catch(() => null);
  if (!response.ok || !payload) throw new Error(payload?.error || `Saved squad request failed with ${response.status}`);
  return payload;
}
...
export function getDefaultBattleSquad(cards = []) {
  return [...cards]
    ...
    .sort((a, b) => Number(b.baseBattlePower || b.battlePower || 0) - Number(a.baseBattlePower || a.battlePower || 0))
...
export function resolveSelectedBattleSquad(cards = [], requestedIds = []) {
  ...
  if (!cleanRequestedIds.length) return { selected: getDefaultBattleSquad(cards), requestedIds: cleanRequestedIds, selectionSource: 'default-highest-power' };
  return { selected, requestedIds: cleanRequestedIds, selectionSource: 'url-query' };
}

export function getSelectedBattleIds(cards = []) { return cards.map(getBattleCardKey).filter(Boolean); }
export function applyBattleEncounterMatchups(cards = [], encounter = {}) { return applyTypeMatchups(cards, encounter); }
export function getBattleSquadPower(cards = [], encounter = null) { const resolvedCards = encounter ? applyBattleEncounterMatchups(cards, encounter) : cards; return resolvedCards.reduce((total, card) => total + Number(card.battlePower || 0), 0); }
...
export function toggleBattleCardSelection({ selectedIds = [], cardId, maxSize = battleSquadMaxSize } = {}) {
  ...
  if (!key) return cleanSelectedIds;
  if (cleanSelectedIds.includes(key)) return cleanSelectedIds.filter((id) => id !== key);
  if (cleanSelectedIds.length >= maxSize) return cleanSelectedIds;
