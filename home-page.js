(function(){
  ...
  function shellCard(r,i){
    var labels={common:'C',uncommon:'U',rare:'R',legendary:'L'};
    return '<article class="homeMiniCard homeRarityShell" style="--r:'+rarityColor(r)+';--a:'+rarityColor(r)+';animation-delay:-'+((i||0)%9*1.7).toFixed(1)+'s"><div class="homeMiniPh">'+labels[r]+'</div></article>';
  }
  function mobileBackgroundColumns(){
    var pool=['common','uncommon','rare','legendary','rare','common','uncommon','legendary','common','rare'];
    var html='';
    for(var c=0;c<2;c++){
      var slice=[];
      ...
      var cardsHtml=stack.map(function(r,i){return shellCard(r,i)}).join('');
      html+='<div class="homeRainCol '+(c%2?'down':'up')+'" style="--speed:'+(36+c*7)+'s">'+cardsHtml+'</div>';
    }
    return '<div class="homeCardRain homeMobileShellRain" aria-hidden="true">'+html+'</div>';
  }
  function backgroundColumns(cards){
    if(isMobileHome())return mobileBackgroundColumns();
