function installAscensionFailsafe(){
  ...
  function showBar(){
    if(suspendRefresh||document.getElementById('ascCeremony'))return;
  ...
  function applyReturnedCard(newCard,result){
  ...
  async function doAscend(id,modal){
    ...
    try{
      ...
      statePatchTimer=setTimeout(()=>{statePatchTimer=null;applyReturnedCard(newCard,result)},1700);
    }catch(e){alert(e.message||'Ascension failed')}
    finally{
      busy=false;
      if(refreshTimer)clearTimeout(refreshTimer);
      refreshTimer=setTimeout(()=>{refreshTimer=null;suspendRefresh=false;refresh()},2500);
    }
  }
