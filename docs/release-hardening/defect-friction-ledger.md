...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RH-001 | Energy | Energy was debited by battle creation but elapsed-time regeneration was never applied, so a depleted player could remain blocked indefinitely. | Reproduced | P1 | Set Energy below cap, advance persisted time, then read resources or create a battle. | Pre-fix resource and battle paths plus interval, cap, concurrency, pre-debit, and user-isolation regression tests. | Shared server-authoritative reconciliation on resource reads and before battle debit; 15 minutes, cap 10, partial preservation, safe timestamp backfill, authenticated-user row isolation. | Sterling explicitly set the rate to 15 minutes on 2026-07-11 | Implemented; preview verification pending |
