...
|---|---|---|---|---|---|
| P0 | Several ownership-sensitive APIs can fall back to a temporary Sterling identity | High | Data may be attributed to the wrong player when session resolution fails | Medium | Remove the fallback domain by domain after adding authenticated contract tests |
...
| P1 | Pull/economy regression coverage is thinner than battle coverage | High | Ticket, daily reset, Gold, or duplicate regressions can affect player resources | Medium | Add D1 adapter tests for daily boundaries, idempotency, five-pulls, and ownership |
