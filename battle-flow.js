const BATTLE_FLOW_ENEMY_TYPES=[
  ...
  {id:'rival_commune',label:'Rival Commune',desc:'Balanced mirror-style opponent squads.',tone:'Mirror',hint:'The closest thing to a rival player squad.'},
...
function battleFlowSteps(active){
  const steps=[['team','Team'],['enemy','Encounter'],['playback','Fight'],['results','Results']];
  const activeIndex=Math.max(0,steps.findIndex(x=>x[0]===active));
  return `<div class="battleFlowSteps">${steps.map(([id,label],i)=>`<span class="${i<activeIndex?'done':i===activeIndex?'on':''}"><b>${i+1}</b>${label}</span>`).join('')}</div>`;
}
...
function battleTeamQuery(){return String(state.battleTeamQ||'').toLowerCase().trim()}
...
function battleFlowFilteredCards(){
  const q=battleTeamQuery(),cid=battleTeamCid(),rar=battleTeamRarity();
  return battleFlowSortedCards().filter(c=>{
    if(cid!=='all'&&c.cid!==cid)return false;
    if(rar!=='all'&&c.rar!==rar)return false;
    if(q&&!`${c.title||''} ${c.tag||''} ${c.effect||''} ${ch(c.cid).name}`.toLowerCase().includes(q))return false;
    return true;
  });
}
function battleFlowSlot(c,i){
  return `<div class="battleFlowSlot ${c?'filled':''}">${c?`<div class="battleSlotMini"><span class="coin" style="--a:${ch(c.cid).a}">${ch(c.cid).in}</span><div><b>${h(c.title)}</b><small>${h(ch(c.cid).name)} · ${h((c.rar||'common').toUpperCase())} · Grade ${h(battleFlowCardScore(c))}</small></div><button class="btn" data-battle-remove="${h(c.id)}" type="button">Remove</button></div>`:`<div class="battleFlowEmpty"><b>Slot ${i+1}</b><span>Choose a card</span></div>`}</div>`
}
function battleFlowTeamFilters(){
  const cid=battleTeamCid(),rar=battleTeamRarity();
  return `<div class="battleTeamFilters"><input id="battleTeamSearch" value="${h(state.battleTeamQ||'')}" placeholder="Search cards..."><select id="battleTeamCharacter"><option value="all" ${cid==='all'?'selected':''}>All Characters</option>${C.map(c=>`<option value="${c.id}" ${cid===c.id?'selected':''}>${h(c.name)}</option>`).join('')}</select><select id="battleTeamRarity"><option value="all" ${rar==='all'?'selected':''}>All Rarities</option><option value="legendary" ${rar==='legendary'?'selected':''}>Legendary</option><option value="rare" ${rar==='rare'?'selected':''}>Rare</option><option value="uncommon" ${rar==='uncommon'?'selected':''}>Uncommon</option><option value="common" ${rar==='common'?'selected':''}>Common</option></select><button class="btn" data-battle-clear-filters type="button">Clear Filters</button></div>`;
}
...
function bindBattleFlow(){
  ...
  document.querySelector('[data-battle-clear-filters]')?.addEventListener('click',()=>{state.battleTeamQ='';state.battleTeamCid='all';state.battleTeamRar='all';render()});
  const search=document.getElementById('battleTeamSearch');if(search&&!search.dataset.ready){search.dataset.ready='1';search.oninput=e=>{state.battleTeamQ=e.target.value;render()}};
  const cid=document.getElementById('battleTeamCharacter');if(cid&&!cid.dataset.ready){cid.dataset.ready='1';cid.onchange=e=>{state.battleTeamCid=e.target.value;render()}};
  const rar=document.getElementById('battleTeamRarity');if(rar&&!rar.dataset.ready){rar.dataset.ready='1';rar.onchange=e=>{state.battleTeamRar=e.target.value;render()}};
...
function injectBattleFlowStyles(){
  ...
  style.textContent=`
.battleFlow{display:grid;gap:18px}.battleFlowTabs{display:flex;gap:8px;flex-wrap:wrap;border:1px solid rgba(255,255,255,.1);border-radius:16px;background:#0b1020;padding:8px}.battleFlowTabs button{border:1px solid rgba(255,255,255,.12);border-radius:999px;background:#151b2e;color:#dfe4ff;padding:10px 13px;font:900 .72rem 'JetBrains Mono',monospace;text-transform:uppercase;letter-spacing:.05em}.battleFlowTabs button.on{background:#f3c93f;border-color:#f3c93f;color:#080b15;box-shadow:0 0 22px rgba(243,201,63,.22)}.battleFlowTabs .battleRulesBtn{color:#f3c93f!important}.battleFlowSteps{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;border:1px solid rgba(255,255,255,.1);border-radius:16px;background:#0b1020;padding:8px}.battleFlowSteps span{border:1px solid rgba(255,255,255,.1);border-radius:999px;background:#151b2e;color:#9fa5bf;padding:8px 10px;font:900 .68rem 'JetBrains Mono',monospace;text-transform:uppercase;display:flex;align-items:center;justify-content:center;gap:7px}.battleFlowSteps b{width:20px;height:20px;border-radius:50%;display:grid;place-items:center;background:#252b40;color:#dfe4ff}.battleFlowSteps span.on{color:#080b15;background:#f3c93f;border-color:#f3c93f}.battleFlowSteps span.on b{background:#080b15;color:#f3c93f}.battleFlowSteps span.done{color:#35d6c5;border-color:rgba(53,214,197,.35)}.battleHomeGrid{display:grid;grid-template-columns:1.2fr 1fr;gap:14px}.battleHero{display:grid;gap:12px;align-content:start;background:radial-gradient(circle at 18% 0,rgba(243,201,63,.2),transparent 34%),linear-gradient(145deg,#11182a,#080d19)}.battleHero h1{margin:0;font:900 clamp(2rem,5vw,3.6rem) Sora,Inter,sans-serif}.battleHero p,.battleLast p{color:#c8ccdf;line-height:1.45}.battleHeroActions{display:flex;gap:10px;flex-wrap:wrap}.battleFlowKicker{font:900 .66rem 'JetBrains Mono',monospace;text-transform:uppercase;letter-spacing:.12em;color:#f3c93f}.battleStats,.battleLast{display:grid;gap:10px}.battleLast{border-left:4px solid rgba(255,255,255,.12)}.battleLast.win{border-left-color:#35d6c5}.battleLast.loss{border-left-color:#ff7b91}.battleLast h3{margin:0;font:900 1.15rem Sora,Inter,sans-serif}.battleFlowLastMeta{display:flex;gap:8px;flex-wrap:wrap}.battleFlowLastMeta span{border:1px solid rgba(255,255,255,.1);border-radius:999px;background:#0c1121;color:#dfe4ff;padding:6px 8px;font:900 .62rem 'JetBrains Mono',monospace}.battleFlowLastMeta span[style]{color:var(--a)}.battleFlowStats{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.battleFlowStats.wide{grid-template-columns:repeat(auto-fit,minmax(135px,1fr))}.battleFlowStats div{border:1px solid rgba(255,255,255,.1);border-radius:14px;background:#0c1121;padding:11px}.battleFlowStats small,.battleFlowHistoryRow small,.battleFlowMiniSquad small,.battleSlotMini small{display:block;color:#9fa5bf;font:900 .62rem 'JetBrains Mono',monospace;text-transform:uppercase;letter-spacing:.06em}.battleFlowStats b{display:block;margin-top:3px;color:#edf1ff}.battleFlowTop{display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap}.battleFlowTop p{margin:4px 0 0;color:#aeb2cc}.battleFlowSelected{position:sticky;top:10px;z-index:18;box-shadow:0 18px 36px rgba(3,5,12,.3)}.battleFlowSlots{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.battleFlowSlot{border:1px solid rgba(255,255,255,.1);border-radius:14px;background:#0b1020;padding:10px;min-height:86px;display:grid;align-items:center}.battleSlotMini{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:10px;align-items:center}.battleSlotMini .coin,.battleFlowMiniSquad .coin{width:36px;height:36px}.battleSlotMini b{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.battleFlowEmpty{text-align:center;color:#9fa5bf}.battleFlowEmpty b{display:block;color:#edf1ff}.battleTeamFilters{display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:10px;margin:12px 0}.battleTeamFilters input,.battleTeamFilters select{border:1px solid rgba(255,255,255,.12);border-radius:12px;background:#0b1020;color:#dfe4ff;padding:11px 12px;font:900 .72rem 'JetBrains Mono',monospace}.battleFlowPickGrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(132px,1fr));gap:12px}.battleFlowPick{position:relative;border:1px solid rgba(255,255,255,.1);border-radius:14px;background:#080d18;color:#dfe4ff;padding:8px;display:grid;gap:6px;text-align:left;cursor:pointer}.battleFlowPick .card{width:100%!important}.battleFlowPick>span:not(.battlePickCheck){font:900 .62rem 'JetBrains Mono',monospace;color:#aeb2cc}.battlePickCheck{position:absolute;z-index:8;left:10px;top:10px;border:1px solid rgba(255,255,255,.16);border-radius:999px;background:rgba(8,10,22,.86);padding:5px 7px;color:#dfe4ff;font:900 .52rem 'JetBrains Mono',monospace;text-transform:uppercase}.battleFlowPick.on{border-color:#f3c93f;box-shadow:0 0 0 2px rgba(243,201,63,.22)}.battleFlowPick.on .battlePickCheck{background:#f3c93f;color:#080b15}.battleFlowEnemyGrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px}.battleFlowEnemy{border:1px solid rgba(255,255,255,.1);border-radius:16px;background:#0b1020;color:#dfe4ff;padding:14px;text-align:left;display:grid;gap:7px;min-height:150px}.battleFlowEnemy span{width:max-content;border:1px solid rgba(255,255,255,.12);border-radius:999px;background:#151b2e;color:#f3c93f;padding:4px 7px;font:900 .56rem 'JetBrains Mono',monospace;text-transform:uppercase}.battleFlowEnemy b{font:900 1.05rem Sora,Inter,sans-serif}.battleFlowEnemy small{color:#aeb2cc;line-height:1.3}.battleFlowEnemy em{color:#dfe4ff;font-style:normal;font-size:.82rem}.battleFlowEnemy.on{border-color:#f3c93f;background:linear-gradient(145deg,rgba(243,201,63,.2),#0b1020);box-shadow:0 0 0 2px rgba(243,201,63,.16)}.battleFlowMiniSquad{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px}.battleFlowMiniSquad div{border:1px solid rgba(255,255,255,.1);border-radius:12px;background:#0c1121;padding:10px;display:grid;grid-template-columns:auto minmax(0,1fr);column-gap:10px;align-items:center}.battleFlowMiniSquad b{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.battleFlowMiniSquad small{grid-column:2}.battleFlowHistoryList{display:grid;gap:8px}.battleFlowHistoryRow{display:grid;grid-template-columns:.75fr 1.45fr 1fr .75fr .55fr .65fr;gap:10px;align-items:center}.battleFlowHistoryRow.win{border-left:4px solid #35d6c5}.battleFlowHistoryRow.loss{border-left:4px solid #ff7b91}.battleHistoryFilters{display:flex;gap:8px;flex-wrap:wrap}.battleHistoryFilters button{border:1px solid rgba(255,255,255,.12);border-radius:999px;background:#151b2e;color:#dfe4ff;padding:8px 12px;font:900 .68rem 'JetBrains Mono',monospace;text-transform:uppercase}.battleHistoryFilters button.on{background:#f3c93f;border-color:#f3c93f;color:#080b15}.battleFlowEmptyPanel{color:#aeb2cc}.battleFlowResultHero{display:flex;align-items:center;gap:14px}.battleFlowResultHero .coin{width:52px;height:52px}.battleFlowResultHero small{display:block;color:#9fa5bf;font:900 .62rem 'JetBrains Mono',monospace;text-transform:uppercase}.battleFlowResultHero b{font:900 1.2rem Sora,Inter,sans-serif}.battleFlowXpList{display:grid;gap:8px}.battleFlowXpList div{display:flex;justify-content:space-between;gap:10px;border:1px solid rgba(255,255,255,.1);border-radius:12px;background:#0c1121;padding:10px}.battleFlowXpList span{color:#35d6c5;font:900 .72rem 'JetBrains Mono',monospace}#battleHistoryPanel{display:none!important}.battleFlow button:disabled{opacity:.45;cursor:not-allowed}@media(max-width:720px){.battleFlowTabs,.battleFlowSteps{display:grid;grid-template-columns:1fr 1fr 1fr}.battleFlowSteps{grid-template-columns:1fr 1fr}.battleFlowTabs button,.battleFlowSteps span{padding:10px 8px}.battleHomeGrid{grid-template-columns:1fr}.battleHeroActions{display:grid}.battleHeroActions .gold,.battleHeroActions .btn{width:100%}.battleFlowSelected{top:6px}.battleFlowSlots{grid-template-columns:1fr}.battleSlotMini{grid-template-columns:auto minmax(0,1fr)}.battleSlotMini .btn{grid-column:1/-1;width:100%}.battleTeamFilters{grid-template-columns:1fr}.battleFlowHistoryRow{grid-template-columns:1fr}.battleFlow .head{display:grid}.battleFlow .head .row{display:grid}.battleFlow .head .row .btn,.battleFlow .head .row .gold{width:100%}.battleFlowPickGrid{grid-template-columns:repeat(auto-fill,minmax(112px,1fr))}.battleFlowEnemyGrid{grid-template-columns:1fr}}
`;
